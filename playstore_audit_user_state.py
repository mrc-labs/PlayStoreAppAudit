...
def load_fresh_cache(
    ...
    for app in apps:
        ...
        if not isinstance(row, dict) or not timestamp:
            continue
        # Cache only healthy, dated listings. Risk/anomaly/error states always run live.
...
def update_cache(rows: list[dict[str, Any]], country: str, language: str) -> None:
    ...
    for row in rows:
        ...
        if not package_name:
            continue
        stored = {k: v for k, v in row.items() if k not in {"criticality", "criticality_key", "criticality_rank", "age_days", "change", "cache_hit"}}
        data[_cache_key(country, language, package_name)] = {
            "fetched_at": now,
            "row": stored,
        }
    # Keep the file bounded. Old/unreadable records are pruned after 45 days.
    cutoff_seconds = 45 * 24 * 3600
    for key in list(data):
        try:
            fetched = datetime.fromisoformat(str(data[key]["fetched_at"]).replace("Z", "+00:00"))
            if (datetime.now(timezone.utc) - fetched.astimezone(timezone.utc)).total_seconds() > cutoff_seconds:
                data.pop(key, None)
        except Exception:
            data.pop(key, None)
