...
v2.0.0 completed the return to Windows x64/ARM64, Linux x64/ARM64 and macOS x64/ARM64 distribution. Windows and Linux shipped unsigned; macOS shipped with ad-hoc engineering signing only. Production-trust signing/notarization remains conditional later-2.x work and must be claimed only after real credential-backed end-to-end validation.

The Local APK / modern LocalAPK successor foundation is now established. v2.1 Track B completed single-file Rename/Remove, Mass Rename with validated metadata templates and collision-safe execution, and safeguarded exact Outdated/Unknown Mass Remove. Duplicate APK detection/management, custom integrations and Windows Explorer integration remain later candidates.

CLI/headless is deferred to v2.2. The next active v2.1 work is the Device Specific resolver PoC, followed by production integration only if that PoC proves sufficiently reliable, then canonical screenshot/help polish.
