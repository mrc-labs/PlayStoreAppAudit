...
The main window has one operational status channel. Its existing `status_label` and progress widget are hosted directly by the native `QStatusBar`; callers continue updating those canonical widgets rather than mirroring state into a second label or progress indicator. The progress widget is shown only while the existing source, audit or finalization flags report active work. Source/device identity remains on its established surfaces, and the status bar adds no preference or persistence state and does not replace the Details Panel.
