...
def install_core_version_support() -> None:
    ...
    def scraper_request(package_name: str, language: str, country: str, config: core.AuditConfig) -> dict[str, Any]:
        try:
            ...
            return {"ok": False, "title": "", "updated": "", "version": "", "error": f"google_play_scraper not installed: {exc}"}
        last_error = ""
        ...
        return {"ok": False, "title": "", "updated": "", "version": "", "error": last_error or "Unknown Google Play scraper error"}
    def fetch_locale(package_name: str, language: str, country: str, config: core.AuditConfig) -> dict[str, Any]:
        ...
        if scraper["ok"] and scraper["updated"]:
            return {
                "status": "available", "http_status": 200, "title": scraper["title"],
                "updated": scraper["updated"], "version": scraper["version"],
                "source": "google_play_scraper",
                "url": f"{core.PLAY_URL}?id={package_name}&hl={language}&gl={country}", "notes": "",
            }
        ...
        if scraper["ok"]:
            ...
            return {
                "status": "available", "http_status": html.get("http_status", 200), "title": title,
                "updated": updated, "version": scraper["version"],
        ...
        return {
            "status": html.get("status", "check_failed"), "http_status": html.get("http_status", ""),
            "title": html.get("title", ""), "updated": html.get("updated", ""), "version": "",
...
def _wait_until_running(pause_event: threading.Event | None, cancel_event: threading.Event | None) -> bool:
    if cancel_event is not None and cancel_event.is_set():
        return False
...
def fetch_app_v8(
    ...
    result: dict[str, Any] = {
        ...
        "play_http_status": primary.get("http_status", ""), "play_title": primary.get("title", ""),
        "play_last_update": primary.get("updated", ""), "play_version": primary.get("version", ""),
        "updated_source": primary.get("source", ""),
        "store_url": primary.get("url", f"{core.PLAY_URL}?id={package_name}"), "notes": primary.get("notes", ""),
    }
    ...
    if not checked:
        ...
        result["notes"] = _append_note(result["notes"], f"selected_country_unavailable:{selected}; also_not_found_in:{checked_text}; likely_removed_or_region_restricted")
    else:
        result["play_status"] = "multi_country_check_inconclusive"
        result["notes"] = _append_note(result["notes"], f"primary_country_status:{primary_status or 'unknown'}; multi_country_checked:{checked_text}; check_errors:{','.join(failed)}")
    return result
...
def audit_apps_v8(
    ...
    def run_one(app: dict[str, str]) -> dict[str, Any] | None:
        if not _wait_until_running(pause_event, cancel_event):
            return None
        return fetch_app_v8(app["app_name"], app["package_name"], config, pause_event=pause_event, cancel_event=cancel_event)
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                results[index] = {
                    ...
                    "play_status": "unexpected_error", "play_http_status": "", "play_title": "",
                    "play_last_update": "", "play_version": "", "updated_source": "",
                    "store_url": f"{core.PLAY_URL}?id={app['package_name']}", "notes": str(exc)[:500],
                }
            completed += 1
...
def enrich_rows_with_device_metadata(rows: list[dict[str, Any]], metadata: dict[str, dict[str, str]]) -> None:
    for row in rows:
...
def dashboard_summary(rows: list[dict[str, Any]], visible_count: int | None = None) -> str:
    ...
    counts = {key: sum(1 for row in rows if str(row.get("criticality_key") or "") == key) for key in ("green", "yellow", "orange", "red", "blue", "purple")}
    parts = [f"{visible_count}/{total} shown" if visible_count is not None and visible_count != total else f"{total} apps"]
    parts.extend([
        f"Current {counts['green']}", f"Aging {counts['yellow']}", f"Stale {counts['orange']}",
        f"Removed {counts['red']}", f"Anomaly {counts['blue']}", f"Other {counts['purple']}",
    ])
    aged = []
    for row in rows:
        try:
