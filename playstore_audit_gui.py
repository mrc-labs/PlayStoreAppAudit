...
class PlayStoreAuditApp(tk.Tk):
    COLUMNS = (
        "app_name", "package_name", "play_status", "play_last_update", "age_days",
        "play_title", "updated_source", "notes", "criticality",
    )
    COLUMN_LABELS = {
        "app_name": "Input name", "package_name": "Package", "play_status": "Play status",
        "play_last_update": "Last update", "age_days": "Age (days)", "play_title": "Play Store title",
        "updated_source": "Update source", "notes": "Notes", "criticality": "Criticality",
    }
    COLUMN_WIDTHS = {
        "app_name": 155, "package_name": 230, "play_status": 170, "play_last_update": 105,
        "age_days": 88, "play_title": 185, "updated_source": 145, "notes": 250, "criticality": 145,
    }

    SYSTEM_COLUMN_NAMES = {
        "is_system", "issystem", "system", "system_app", "systemapp", "is_system_app",
        "issystemapp", "app_type", "apptype", "type",
    }
    PACKAGE_COLUMN_NAMES = {"package", "packageid", "packagename", "package_name", "appid", "app_id", "id"}
    TRUE_SYSTEM_VALUES = {"1", "true", "yes", "y", "system", "system_app", "systemapp", "preinstalled", "pre-installed"}
    FALSE_SYSTEM_VALUES = {"0", "false", "no", "n", "user", "user_app", "userapp", "third-party", "third_party", "thirdparty"}
    ...
    DEFINITE_SYSTEM_PREFIXES = (
        ...
        "com.google.android.ext.", "com.google.android.networkstack", "com.google.android.adservices.api",
    ...
    MONTHS = {
        "jan": 1, "january": 1, "gen": 1, "gennaio": 1, "feb": 2, "february": 2, "febbraio": 2,
        "mar": 3, "march": 3, "marzo": 3, "apr": 4, "april": 4, "aprile": 4,
        "may": 5, "maggio": 5, "mag": 5, "jun": 6, "june": 6, "giu": 6, "giugno": 6,
        "jul": 7, "july": 7, "lug": 7, "luglio": 7, "aug": 8, "august": 8, "ago": 8, "agosto": 8,
        "sep": 9, "sept": 9, "september": 9, "set": 9, "settembre": 9,
        "oct": 10, "october": 10, "ott": 10, "ottobre": 10,
        "nov": 11, "november": 11, "novembre": 11, "dec": 12, "december": 12, "dic": 12, "dicembre": 12,
    }
    ...
    def __init__(self) -> None:
        ...
        self.skip_system_var = tk.BooleanVar(value=False)
        ...
        self.criticality_count_vars = {
            key: tk.StringVar(value=f"{info['short']} 0") for key, info in self.CRITICALITY.items()
        }
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Spinbox(options, from_=1, to=32, textvariable=self.workers_var, width=7).grid(row=0, column=3, padx=(5, 18))
        ttk.Label(options, text="Country controls the Play Store market used for the availability check.").grid(row=0, column=4, sticky="w", padx=(8, 12))
        self.advanced_button = ttk.Button(options, text="Advanced ▸", command=self._toggle_advanced)
        self.advanced_button.grid(row=0, column=5, sticky="e")
        options.columnconfigure(4, weight=1)

        self.advanced_frame = ttk.LabelFrame(root, text="Advanced", padding=10)
        self.advanced_frame.columnconfigure(5, weight=1)
        ttk.Label(self.advanced_frame, text="Store language:").grid(row=0, column=0, sticky="w")
        ttk.Entry(self.advanced_frame, textvariable=self.language_var, width=7).grid(row=0, column=1, padx=(5, 16))
        ttk.Label(self.advanced_frame, text="Mostly affects localised titles/text, not which country Store is checked.").grid(row=0, column=2, sticky="w", padx=(0, 24))
        ttk.Checkbutton(self.advanced_frame, text="Skip system apps during audit", variable=self.skip_system_var).grid(row=0, column=3, sticky="w")
        ttk.Label(self.advanced_frame, text="Faster, but skipped system apps will not be analysed or appear in results.").grid(row=0, column=4, sticky="w", padx=(8, 0))
        ...
        progress_frame.grid(row=5, column=0, sticky="ew", pady=(10, 8))
        ...
        for key in ("red", "orange", "yellow", "blue", "purple", "green"):
            ...
            tk.Button(
                legend, textvariable=self.criticality_count_vars[key], command=lambda k=key: self._set_criticality_filter(k),
                relief="flat", bd=1, padx=8, pady=3, background=info["background"], foreground=info["foreground"],
                activebackground=info["background"], activeforeground=info["foreground"], font=("Segoe UI", 9), cursor="hand2",
            ).pack(side="left", padx=3)

        ttk.Label(
            results,
            text=("Click a colour counter to filter. Removed = not on Store | Aging = >365–730d | "
    ...
    def _choose_input(self) -> None:
        selected = filedialog.askopenfilename(title="Choose app list", filetypes=[("App lists", "*.csv *.tsv *.txt"), ("CSV", "*.csv"), ("Text", "*.txt"), ("All files", "*.*")])
        if not selected:
            return
        try:
    ...
    def _parse_adb_packages(output: str) -> set[str]:
        return {line.replace("package:", "", 1).strip() for line in output.splitlines() if line.strip().startswith("package:")}
    ...
    def _is_definite_system_package(self, package_name: str) -> bool:
        return package_name in self.DEFINITE_SYSTEM_PACKAGES or any(package_name.startswith(prefix) for prefix in self.DEFINITE_SYSTEM_PREFIXES)
    ...
    def _start_audit(self) -> None:
        ...
        if self.skip_system_var.get():
            ...
            if not apps:
                messagebox.showerror("Nothing to audit", "All packages were classified as system apps and skipped by Advanced settings.")
                return
        ...
        self.source_var.set(f"{source_label} source: {len(all_apps)} packages | {len(system_packages)} classified as system | {classification_method}{extra}")
        self.run_button.config(state="disabled")
        self.export_button.config(state="disabled")
        self.progress["value"] = 0
        self.progress["maximum"] = len(apps)
        self.status_var.set(f"Starting audit for {len(apps)} packages in Store country '{country}'" + (f" ({skipped_system} system apps skipped)…" if skipped_system else "…"))
        self.current_rows = []
        self._refresh_table()
        config = AuditConfig(country=country, language=language, max_workers=workers)
    ...
    def _set_criticality_filter(self, key: str | None) -> None:
        self.criticality_filter = None if key is not None and self.criticality_filter == key else key
        self._refresh_table()
    ...
    def _rows_before_criticality_filter(self) -> list[dict[str, object]]:
        ...
        if query:
            ...
            rows = [row for row in rows if query in " ".join(str(row.get(column, "") or "") for column in search_fields).casefold()]
        return rows
    ...
    def _refresh_table(self) -> None:
        ...
        counts = {key: sum(1 for row in count_rows if row.get("criticality_key") == key) for key in self.CRITICALITY}
    ...
    def _export_results(self) -> None:
        if not self.current_rows:
            return
        selected = filedialog.asksaveasfilename(title="Export audit results", defaultextension=".csv", filetypes=[("CSV", "*.csv")], initialfile="playstore_audit_results.csv")
        if not selected:
            return
        try:
